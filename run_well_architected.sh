...
# ArchiQ - AWS 리소스 기반 Well-Architected 리뷰
# 사용법: ./run_well_architected.sh [-r region]
...
PROMPT_FILE="${SCRIPT_DIR}/src/prompt/well_architected_review.md"
OUTPUT_DIR="${SCRIPT_DIR}/output/well-architected"
...
# 도움말 함수
show_help() {
    echo -e "${BLUE}ArchiQ - AWS 리소스 기반 Well-Architected 리뷰${NC}"
    echo ""
    echo "사용법: $0 [옵션]"
    echo ""
    echo "옵션:"
...
main() {
    ...
    echo -e "${YELLOW}🏗️ AWS 리소스 기반 Well-Architected 리뷰 실행 중...${NC}"
    echo ""
    ...
    if [ $? -eq 0 ]; then
        echo ""
        echo -e "${GREEN}✅ AWS 리소스 기반 Well-Architected 리뷰 완료!${NC}"
        echo -e "${BLUE}🏗️ 결과는 output/well-architected/ 디렉토리에서 확인하세요.${NC}"
    else
