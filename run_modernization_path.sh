...
# ArchiQ - AWS 리소스 기반 현대화 경로 분석
# 사용법: ./run_modernization_path.sh [-r region]
...
PROMPT_FILE="${SCRIPT_DIR}/src/prompt/modernization_path.md"
OUTPUT_DIR="${SCRIPT_DIR}/output/modernization"
...
# 도움말 함수
show_help() {
    echo -e "${BLUE}ArchiQ - AWS 리소스 기반 현대화 경로 분석${NC}"
    echo ""
    echo "사용법: $0 [옵션]"
    echo ""
    echo "옵션:"
...
main() {
    ...
    echo -e "${YELLOW}🚀 AWS 리소스 기반 현대화 경로 분석 실행 중...${NC}"
    echo ""
    ...
    if [ $? -eq 0 ]; then
        echo ""
        echo -e "${GREEN}✅ AWS 리소스 기반 현대화 경로 분석 완료!${NC}"
        echo -e "${BLUE}📊 결과는 output/modernization/ 디렉토리에서 확인하세요.${NC}"
    else
